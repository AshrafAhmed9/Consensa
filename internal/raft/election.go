...
func (n *node) startPreVote() {
	...
	// Only current voters are asked -- during the joint phase that's Old ∪ New
	// (votingPeers), not just Old, since HasQuorum needs a majority of New to ever be
	// reachable too. A non-voter's response could never legitimately count, so messaging
	// one would just be a wasted round trip.
...
func (n *node) handlePreVote(m Message) {
	// A non-voter never grants a vote, even a pre-vote, even if a leader ever mistakenly
	...
	// HasQuorum the way a real voter's grant can. Defense in depth -- startPreVote/
	// startElection never message a non-voter in the first place -- not a load-bearing
	// path in the current code, but a non-voter voting would silently break the
	// "only voters count toward HasQuorum" invariant everywhere else in this file
	// relies on.
	if !n.isVoter(n.id) {
...
func (n *node) handleVote(m Message) {
	// See handlePreVote's comment: a non-voter never grants a real vote either, for the
	// identical reason.
	if !n.isVoter(n.id) {
