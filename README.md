...
To stage an already-addressable replica as a non-voting learner, include it in the same
`--peers` map on every process and pass `--learners <id>` on every process. The durable
vector and KV replicas expose `ProposeConfChange(voters, learners)` for an operator layer
to promote, demote, or remove those known IDs through Raft joint consensus. The shipped
binary intentionally does not expose that operation unauthenticated over gRPC or HTTP;
bootstrap, authenticated administration, and client routing updates are still separate
work.
...
- **Learners and joint consensus are wired into the live quorum path.** `Config.Learners`
  ...
  smaller first step made it possible to promote already-known, caught-up learners using
  log-replicated two-phase joint consensus. During the transition, elections and commits
  require a majority of both configurations; config state is applied on append, carried
  through snapshots, and finalized automatically after the joint entry commits.
...
ranges' messages are still never coalesced into a single wire frame; joint consensus can
promote, demote, or remove replicas already present in a group's static transport peer
universe, but there is not yet a client/admin workflow for provisioning a genuinely new
process, publishing its address, and changing range routing; serializable isolation is partially closed (Phase 14 -- both `Store` and `DurableStore`
